                ...
                <div className={styles.category}>
                    ...
                    <div className={styles.personalInformation}>
                        ...
                        <div className="relative order-1 lg:order-none">
                            <input type="text" value={address ? address : ''} id="name" className={`block py-4 px-[14px] w-full text-[15px] text-gray-900 bg-transparent rounded-[16px] border-1 ${errors?.name ? 'border-red-500' : 'border-gray5'} appearance-none focus:outline-none focus:ring-2 focus:ring-gray-300 focus:border-transparent peer`} placeholder=" " disabled />
                            <label htmlFor="name" className="absolute text-[15px] text-[#C0C0C0] duration-300 transform -translate-y-4 scale-100 top-1.5 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-[#C0C0C0] peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-1.5 peer-focus:scale-100 peer-focus:-translate-y-4 left-1 cursor-text">{lang?.['Адрес']}</label>
                        </div>
                        <div className='order-2 lg:order-none'>
                            <button className={`${errors?.address_id ? 'border-red-500' : 'border-accent'} py-4 w-full h-full ${errors?.address_id ? 'text-red-500' : 'text-accent'} border rounded-[16px] hover:text-white hover:bg-accent trnasition duration-300 font-semibold`} type="button" onClick={() => setAddressOpen(true)}>langВыберите адрес</button>
                        </div>
                        <div className="relative">
